...
| 요소 | 형태 | 크기 | 위치 |
|---|---|---|---|
| 앨범(음반) | **정사각**, radius 4 | 52 | 좌상단 |
| 사람 | **원형**, 배경색 2px 링 | 24 | 우측 세로 스택, 앨범 우변에 12px 겹침 |
| 슬롯 전체 | — | 64 × 52 | — |
...
- 원 하나에 호버하면 **1.4배로 커지고 브라스 링**이 붙습니다. `transform-origin: right center`라 **오른쪽 변이 고정되어 옆 텍스트를 침범하지 않습니다.** 오른쪽으로 키우면 실측 여유가 3.5px까지 좁아집니다.
- 각 원에는 **역할이 붙은 `title`**을 답니다(`작곡가 · 라흐마니노프`). 아이콘 단독 버튼과 같은 규칙입니다.
- 슬롯이 56 → 64로 넓어져도 `.pl-l`이 고정 폭(296 / 좁은 화면 228)이라 바깥 레이아웃은 그대로입니다.
...
| 지휘자 | `PerformanceCredit(role: 'conductor')` | **받을 수 없습니다.** `/sectors/{id}/performances`는 `artistId`만 주고 credits를 실은 계약 엔드포인트는 404입니다 → **B16** |
