...
구간 라벨에 A·B·C·D를 쓰지 않습니다. 곡마다 구성이 달라 **항목별로만 의미가 있는 임의 라벨**이기 때문입니다.

한동안 **음표 값**(8분·4분·2분·온음표)으로 구간 길이를 표시했지만 교체했습니다. 음표는 *얼마나 긴가*만 말하고 ***곡 어디쯤인가*를 못 말합니다.** 비교하려면 두 가지가 다 필요합니다.

**곡 전체를 오선 한 줄로 펴고 그 위에 구간을 앉힙니다.**
...
| 요소 | 규격 |
|---|---|
| 오선 | 5줄, 줄 간격 **10**(모바일 6), 색 `foreground-faint` |
| 머리 | 음자리표 · 조표 · 박자표. 왼쪽 74px(모바일 46) |
| 빠르기말 | 오선 위, **이탤릭 세리프** 10.5px. **악장마다 따로** 붙습니다 (라흐 3번: `Allegro ma non tanto` / `Intermezzo. Adagio` / `Finale. Alla breve`) |
| 악장 경계 | **겹세로줄**(얇은 줄 2개, 간격 3px) |
| 구간 | 브라스 채움 + 좌우 2px 인셋 테두리. 선택 시 불투명도 .2 → .6 |
| 악장 라벨 | 오선 아래 10.5px 모노 |

- **가로 위치와 폭은 구간의 실제 `startMs` / `endMs`** 입니다. 길이가 곧 폭이라 음표 값이 필요 없습니다.
- 타임라인 길이는 **마지막 구간의 끝**입니다. `Piece.durationMinutes`를 쓰지 않는 이유는 실연 영상이 그보다 길기 때문입니다(라흐 3번 표기 40분, 임윤찬 실연 42:32).
- 연주자마다 시각이 다르므로 **한 연주를 기준으로 그리고 A/B를 바꾸면 다시 계산**합니다.
- 좁은 구간은 실제로 아주 좁습니다(최소 26px). 폭을 부풀려 왜곡하지 않고, 투명 의사요소로 **클릭 영역만 44px로 확보**합니다.
- 계산은 `lib/design/section-timeline.ts`의 `placeSections()` / `movementBoundaries()`입니다.
- **구간 블록은 1번선과 5번선 *사이*에 넣습니다**(`top: 1px; height: 39px`). 오선 밖으로 튀어나와도, 오선 높이에 딱 맞춰도 어긋나 보입니다. 딱 맞추면 바깥 두 줄이 **블록 테두리를 겸하게 되어** 구간 밖에서는 회색 선, 안에서는 금색 면의 경계로 성격이 바뀝니다. 사이에 넣으면 두 줄이 어디서나 그냥 선으로 남습니다.
- **구간 블록에 `border-radius`를 주지 않습니다.** 모서리가 하필 1번선·5번선 위에 앉아 선 끝을 4px 얼룩으로 번지게 합니다.

**구간 안은 비워 둡니다.**

한때 구간 안을 빠르기에 맞춘 음표 패턴으로 채웠지만 걷어냈습니다. 두 가지 이유입니다.

- **그 대목의 진짜 음표가 아닙니다.** 악보처럼 보이는데 실제 악보가 아니면 정보가 아니라 오해를 만듭니다.
- **읽히지도 않습니다.** 구간 폭이 데스크톱 26~107px, 모바일 6~25px입니다. 이 폭에서 밀도로 빠르기를 읽어내는 사람은 없습니다.

오선이 이미 위치와 길이를 말하고 있으므로 구간은 **빈 브라스 블록**으로 충분합니다.

**대신 길이 비교는 영상 슬롯 안에 둡니다.**

같은 악절인데 연주자마다 길이가 다르다는 것이 이 제품의 핵심인데, 그동안 하단 재생 바의 시크 트랙에만 얇게 들어가 있었습니다. 슬롯 썸네일 아래에 **4px 길이 막대**를 두고 그 구간 최장 연주를 100%로 잡아 채웁니다(1악장 카덴챠 실측: 임윤찬 1:41 = 68.2% / 랑랑 2:28 = 100% / 유자 왕 1:32 = 62.2%).

- 세 슬롯이 같은 폭이라 **채움 비율만으로 바로 비교**됩니다.
- 오선 아래에 별도 블록으로 두었더니 **112px를 먹어 첫 화면에서 영상이 15.5%만 남았습니다.** 슬롯 안으로 옮기면 세로 비용이 14px입니다. "영상이 화면의 주인"이라는 원칙이 이 화면에서는 다른 무엇보다 앞섭니다.
- 연주자 이름 줄 오른쪽에 길이를 모노로 붙이고, 막대에는 차이를 `title`로 답니다.

#### 4.7.6 악보 글리프는 직접 그리지 않는다

음자리표·조표·박자표는 **Bravura**를 씁니다. Steinberg가 만든 **SMuFL 레퍼런스 폰트**이고 SIL OFL이라 재배포가 됩니다.

- 직접 그린 음자리표는 작은 크기에서 반드시 무너집니다. 그리고 악보를 읽는 사람에게는 형태가 조금만 달라도 **틀린 기호**로 보입니다. 고유 아이콘을 만드는 기준(4.7.1)은 "대응물이 없는 개념"인데, 음자리표는 대응물이 있는 정도가 아니라 **표준이 정해져 있는 기호**입니다.
- **SMuFL 규약: 오선 한 칸 = `0.25em`.** 줄 간격 10px이면 `font-size`는 정확히 40px입니다 (`staffFontSize()`).
- 글리프 원점은 baseline입니다. 높은음자리표는 **G선**(아래서 둘째 줄), 조표와 박자표는 해당 음 위치에 baseline을 둡니다.
- **높은음자리표는 오선 위아래로 크게 벗어납니다.** 캔버스를 오선 높이로만 잡으면 잘립니다(시안에서 41.7% 잘렸습니다). 세로로 넉넉히 잡고 절대배치로 띄웁니다.
- 코드: `lib/design/smufl.ts`(글리프·조표 계산), `components/ui/notation-glyph.tsx`(렌더).
- 폰트 로드는 네이티브 `expo-font`, 웹 `@font-face`입니다. **`Bravura.woff2`(323KB)와 OFL 라이선스 파일을 저장소에 함께 넣어야 합니다.**

**데이터 현황**: `Piece`에 **조성·빠르기말·박자·악장 경계가 없습니다.** 조성은 제목 문자열(`피아노 협주곡 3번 D단조`)에만 들어 있고, 나머지는 어디에도 없습니다. **제목을 파싱하거나 곡 상식으로 채우지 않습니다** → **B24**. 없으면 머리를 음자리표만 그리고 빠르기말은 생략합니다.
...
### 6.5 재생 바 — 지금 재생 중인 것이 무엇인가

**아트워크 슬롯은 "앨범 한 장과 거기 참여한 사람들"입니다. 작곡가 초상 한 장이 아닙니다.**

"임윤찬이 라흐마니노프 3번을 친다"에는 최소 세 개의 정체가 들어 있습니다 — **무슨 음반인가 · 누가 썼는가 · 누가 연주하는가.** 초상 한 장만 두면 이 중 둘이 사라지고, 특히 **연주자는 이 제품에서 바뀌는 축**이라 빠지면 안 됩니다.

| 요소 | 형태 | 크기 | 위치 |
|---|---|---|---|
| 앨범(음반) | **정사각**, radius 4 | 52 | 좌상단, `z-index: 2` |
| 사람 | **원형**, 배경색 2px 링 | 24 | 세로 스택, **앨범 뒤(`z-index: 1`)** |
| 슬롯 전체 | — | 74 × 52 (좁은 화면 66) | — |

**사람 원의 순서는 위에서부터 `작곡가 → 지휘자 → 연주자`입니다.** 쓴 사람에서 켠 사람으로 내려옵니다. 겹침에서 **맨 아래(연주자)가 가장 앞에 옵니다** — 바뀌는 축이 가장 앞입니다.

- **네모는 음반, 원은 사람.** 4장 형태 규칙 그대로라 라벨 없이 구분됩니다.
- 인원수에 따라 **원 크기는 24로 고정하고 간격만 바꿉니다.** 2명이면 `gap 4`(24+4+24=52), 3명이면 `margin-top:-10`(24+14+14=52). 어느 쪽이든 스택 높이가 앨범과 똑같은 52입니다.
- **평소에는 사람이 앨범 뒤에 꽂혀 있습니다.** 앨범 우변 너머로 오른쪽 8px만 보입니다 — CD가 케이스에 꽂혀 있는 모습 그대로입니다.
- **슬롯에 마우스를 올리면 CD를 빼내듯 오른쪽으로 밀려 나옵니다.** `margin-left` 36 → 50 (14px), `cubic-bezier(.16,1,.3,1)` 240ms. **아래(연주자)부터 빠져나오도록** 위쪽 원에 50ms씩 지연을 겁니다 — 바뀌는 축이 먼저 보입니다.
- 나온 상태의 오른쪽 끝이 정확히 슬롯 폭(74)이라 **옆 텍스트를 침범하지 않습니다.**
- 개별 원에 호버하면 브라스 링이 붙습니다. 크기는 키우지 않습니다 — 이미 밀려 나와 있어 더 키우면 텍스트를 넘습니다.
- 각 원에는 **역할이 붙은 `title`**을 답니다(`작곡가 · 라흐마니노프`). 아이콘 단독 버튼과 같은 규칙입니다.
- 슬롯이 넓어져도 `.pl-l`이 고정 폭(296)이라 바깥 레이아웃은 그대로입니다. 6개 탭 전부 `.player` 1238 / `.pl-r` x=1011.5로 동일한 것을 실측했습니다.
- **1120px 이하에서는 `.pl-l`이 228로 줄어 곡명 `.s`가 잘립니다.** 아트워크가 56px이던 때부터 있던 문제입니다. 두 가지로 완화합니다.
  1. 슬롯을 74 → **66**으로 줄입니다(호버 이동 거리도 14 → 6px).
  2. **좌측 북마크 버튼을 접습니다**(44px 회수). 같은 동작이 레퍼토리 탭에 있으므로 기능이 사라지지 않습니다.
  - 그래도 남는 말줄임은 받아들입니다. 잘리는 쪽은 항상 `.s`이고 **`.t`(연주자)는 유지합니다** — 바뀌는 축이라 먼저 지킵니다.
- **모바일 미니도 같은 규칙**(앨범 40 + 원 19)이되 호버는 걸지 않습니다. **비교 화면의 미니는 예외** — 이미 그 작품 안에 들어와 있고 우측에 A/B 연주자 토글이 있으므로 연주자 원형 하나만 둡니다.

**데이터 현황 (2026-09-16 실측)**

| 요소 | 소스 | 상태 |
|---|---|---|
| 앨범 표지 | `Recording.coverUrl` | **이미지는 있습니다.** 임윤찬(`artistId` 187) 음반 12건이 실제로 오고, 그중 `Rachmaninoff: Piano Concerto No. 3 (Live from the 2022 Cliburn Competition)` (Decca, 2025)가 시안의 곡과 정확히 일치합니다 |
| 앨범 ↔ 연주 연결 | **없음** | `Performance`에도 `ComparisonPerformance`에도 `recordingId`가 없습니다. 제목 문자열로 맞추는 건 추측이므로 하지 않습니다 → **B22** |
| 작곡가 | `Piece.composerId` → `Composer.avatarUrl` | 사용 가능 |
| 연주자 | `Performance.artistId` → `Artist.imageUrl` | 사용 가능 |
| 지휘자 | `PerformanceCredit(role: 'conductor')` | **받을 수 없습니다.** `/sectors/{id}/performances`는 `artistId`만 주고 credits를 실은 계약 엔드포인트는 404입니다 → **B23**(B16에서 따로 뺀 항목). 협주곡은 독주자만으로 성립하지 않으므로 우선순위 높음 |

- `Piece`에는 이미지 필드가 **아예 없습니다.** 지금 화면들이 작품 아트워크로 쓰는 이미지는 전부 **작곡가 초상**입니다. 그래서 앨범 칸과 작곡가 원에 같은 그림이 들어가는 중복이 생깁니다. B22가 풀리기 전까지는 앨범 칸을 **작곡가 초상으로 폴백**하되, 폴백일 때는 작곡가 원을 **숨깁니다**(같은 그림을 두 번 보여 주지 않습니다).
- 지휘자 원은 B16 이후에 켭니다. 그전까지는 2명 배치입니다. **제목 문자열이나 상식으로 지휘자를 채워 넣지 않습니다.**
...
**모바일 홈 구성 확정 (2026-09-16 시안)**

시안 초안의 모바일 홈은 섹션이 3개뿐이라 **데스크톱 홈(셸프 5개)보다 얇았고, 무엇보다 홈에서 "비교"가 보이지 않았습니다.** 제품의 차별점이 홈에 없으면 그냥 음악 앱으로 읽힙니다. 다음 순서로 확정합니다.

| 순서 | 섹션 | 형태 | 근거 |
|---|---|---|---|
| 1 | 인사말 + 프로필 | 한 줄 | 애니메이션만 제거하고 줄은 남깁니다. 프로필 진입점을 겸합니다 |
| 2 | **오늘의 비교** | 16:9 영상 1장(전체 폭) + 영상 위 A/B 아바타 토글 + 아래 구간 음표·연주자별 길이 | 홈에서 제품을 한 번에 설명합니다. 영상은 크게, 정보는 그 위에 얹습니다 |
| 3 | 최근 본 작품 | 2열 타일 | 재방문 동선 |
| 4 | 비교할 수 있는 작품 | 3열 | 전체 8곡 |
| 5 | **연주자** | 원형 가로 스크롤 72px | 데스크톱 홈엔 있는데 모바일엔 없었습니다 |
| 6 | **작곡가** | 원형 가로 스크롤 72px | 상동 |
| 7 | **시대로 듣기** | 2×2 타일, 초상을 배경으로 눕히고 연대 범위 표기 | 실재하는 4종. 작곡가 수는 계속 바뀌므로 대신 **연대 범위**를 씁니다 |
| 8 | 이번 주 공연 | 3열 + 제목·일시 | 전용 탭이 생겼으므로 홈에서는 맨 아래 |

- 스펙 원안과 다른 점 둘: **인사말을 지우고 검색 바를 넣는 안은 채택하지 않았습니다**(검색은 탭바에 있음). **공연을 위쪽이 아니라 맨 아래**에 둡니다(공연 탭이 따로 있음).
- 시대 타일에는 **작곡가 수를 쓰지 않습니다.** 2차 조사에서 규모가 3배로 변한 것처럼 계속 바뀌는 값이고, 시대별 분포는 확정 집계가 없습니다. 연대 범위는 변하지 않습니다.
- 가로 스크롤 셸프는 `.mbd` 좌우 패딩을 뚫고 **화면 끝까지 흘립니다**(edge-to-edge). 패딩 안에 가두면 스크롤 여지가 있다는 게 안 보입니다.
- 실측: 본문 세로 1,315px(화면 568px의 2.3배), 가로 넘침 0, 의도된 가로 스크롤은 셸프 2개뿐.
...
- **정렬은 필터 축이 아닙니다.** 필터 그룹들을 감싸는 좌측 영역(내부에서 접힘)과 정렬 버튼을 두는 우측 영역으로 나누고, 바깥은 줄바꿈하지 않습니다. 한 덩어리로 두면 정렬 버튼 하나 때문에 둘째 줄이 통째로 비는 일이 생깁니다. 1000~1440px에서 정렬 버튼이 항상 첫 칩 줄에 우측 정렬로 남는 것을 실측으로 확인했습니다.
...
**검색 API 실동작 (2026-09-17 실측)**

| 대상 | 엔드포인트 | 상태 |
|---|---|---|
| 작곡가 | `/composers/search?q=` | 동작. **한글·영문 모두 매칭**(`Rach` → 라흐마니노프) |
| 연주자 | `/artists/search?q=` | 동작. 악기명도 매칭(`피아` → 피아니스트들) |
| 공연 | `/concerts/search?q=` | 동작 |
| **작품** | `/pieces/search?q=` | **404. 없습니다** → B25 |

- 목록 엔드포인트(`/composers?q=`)는 **질의를 무시하고 전체를 돌려줍니다.** 반드시 `/search` 경로를 씁니다.
- **관련도 정렬이 없습니다.** `/artists/search?q=임`이 뉴욕 필하모닉·사이먼 래틀을 먼저 주고 임윤찬이 4번째입니다 → B26.

**결과 화면 구성**

1. 질의 바 — 활성 테두리(brass) + 커서. 우측에 `⌘K` 힌트
2. 타입 칩 — `전체 / 작곡가 / 작품 / 연주자 / 공연`, 각 개수 표시
3. **가장 근접** 카드 1장 + **비교할 수 있는 작품** 목록을 2열로 나란히
4. 연주자 행(원형) → 공연 행(포스터)

- **"가장 근접"에 비교 보유 배지를 답니다**(`비교 2곡 · 연주 21`). 검색해서 찾은 다음 "비교가 있나?"를 다시 확인해야 하면 동선이 끊깁니다.
- **작품 그룹은 작곡가 일치에서 파생합니다.** B25 전까지는 이름으로 작품을 찾을 수 없으므로, 작곡가가 맞으면 그 작곡가의 비교 보유 작품을 같이 보여 줍니다. **이 우회를 제목 문자열 매칭으로 대체하지 않습니다.**
- 검색 결과에도 **폴백 타일이 그대로 나옵니다**(호로비츠 등). 아티스트 이미지 보유가 48%라 예외가 아니라 기본입니다.

**커맨드 팔레트는 6.3절 규격 그대로**입니다. 시안에서 `⌘K` / `Ctrl+K` / 버튼 / `Esc`가 실제로 동작합니다.
...
| D-14 | 오선 위 강조 구간이 "어긋나 보인다". 좌표는 한 픽셀도 안 틀림 | 블록 높이가 오선 전체 높이와 같아 **바깥 두 줄이 블록 테두리를 겸함**. 같은 y인데 구간 밖은 대비 3.25:1의 선, 안은 1.96:1의 면 경계라 눈이 끊김으로 읽음. `border-radius: 2px`가 그 두 줄 위에서 4px 얼룩을 만들어 가중 | 강조 면은 **선 사이**에 넣고 모서리 반경을 0으로. 선과 면이 같은 픽셀 행을 공유하지 않게 합니다 |
