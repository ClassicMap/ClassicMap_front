...
| 구간 음표 | 구간 안을 채우는 반복 패턴. 빠를수록 촘촘합니다 |
...
- 그 악장의 빠르기에 따라 밀도를 바꿉니다 — 빠르면 촘촘하게 이어 붙인 8분음표, 느리면 빈 머리 음표를 띄엄띄엄.
- **이것은 실제 악보가 아니라 빠르기의 질감입니다.** 그 대목의 진짜 음표가 아니므로 화면 어디에서도 악보라고 부르지 않습니다. 실제 악보 데이터가 생기면 이 자리를 그대로 대체하면 됩니다.
- 구현은 반복 배경이 아니라 **`mask-image` + `background-color: currentColor`** 입니다. 색이 테마와 선택 상태를 그대로 따라가서 타일을 여러 벌 만들 필요가 없습니다.
