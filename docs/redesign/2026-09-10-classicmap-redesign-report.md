...
**아트워크 슬롯에는 작품과 연주자를 같이 둡니다. 작곡가가 아닙니다.**

"임윤찬이 라흐마니노프 3번을 친다"에서 재생 중인 것은 **작품 하나와 연주 하나**입니다. 작곡가 초상 한 장만 두면 지금 누가 연주하는지가 사라지고, 연주자 사진만 두면 무슨 곡인지가 사라집니다. 이 제품에서는 **연주자가 바뀌는 축**이므로 더더욱 빠지면 안 됩니다.

| 요소 | 형태 | 크기 | 위치 |
|---|---|---|---|
| 작품(앨범) | **정사각**, radius 4 | 52 | 좌상단 |
| 연주자 | **원형**, 배경색 2px 링 | 28 | 우하단, 작품 위에 겹침 |
| 슬롯 전체 | — | 68 × 52 | — |

- **네모는 콘텐츠, 원은 사람.** 4장 형태 규칙 그대로라 라벨 없이도 둘이 구분됩니다.
- **호버하면 연주자가 앞으로 나옵니다.** 원이 28 → 38로 커지고 6px 떠오르며 브라스 링이 붙고, 작품 쪽은 `brightness(.55)`로 물러납니다.
- **호버 확대는 오른쪽 변을 고정하고 왼쪽·위로만 키웁니다.** 오른쪽으로 키우면 옆 텍스트를 침범합니다(실측 여유가 3.5px까지 좁아졌습니다). 오른쪽 변 고정이면 침범이 0이고, "앞으로 나온다"는 인상도 더 맞습니다.
- 슬롯이 56 → 68로 넓어져도 `.pl-l`이 고정 폭(296 / 좁은 화면 228)이라 **바깥 레이아웃은 그대로입니다.** 6개 탭 전부 `.player` 1238 / `.pl-r` x=1011.5로 동일한 것을 실측했습니다.
- **모바일 미니 플레이어도 같은 규칙**을 쓰되(작품 40 + 연주자 22) 호버 효과는 걸지 않습니다. 단, **비교 화면의 미니는 예외**입니다. 이미 그 작품 안에 들어와 있고 우측에 A/B 연주자 토글이 있으므로, 아트워크는 연주자 원형 하나만 둡니다.
